    ...
    companion object {
        ...
        private const val FRAGMENT_SHADER_SRC = """
            ...
            void main() {
                ...
                if (p < 0.10) {
                    // 阶段 1：清晨初升曦光 (0.00 ~ 0.10) - 提升日出太阳光强度与高亮白点值，高透金赤朝阳 (2800K)
                    float t = p / 0.10;
                    colCore  = mix(vec3(1.0, 1.0, 1.0), vec3(1.0, 0.98, 0.90), t);
                    colMid   = mix(vec3(1.0, 0.55, 0.20), vec3(1.0, 0.68, 0.26), t);
                    colOuter = mix(vec3(1.0, 0.32, 0.08), vec3(1.0, 0.48, 0.12), t);
                    rayScale = mix(0.02, 0.10, t);
                    k1 = 16.0;
                    k2 = 50.0;
                } else if (p < 0.25) {
                    // 阶段 2：旭日东升跃金 (0.10 ~ 0.25) - 高度角快速爬升，白点高照，璀璨金橙与暖金 (3800K ~ 4800K)
                    float t = (p - 0.10) / 0.15;
                    colCore  = mix(vec3(1.0, 0.98, 0.90), vec3(1.0, 1.0, 0.98), t);
                    colMid   = mix(vec3(1.0, 0.68, 0.26), vec3(1.0, 0.86, 0.42), t);
                    colOuter = mix(vec3(1.0, 0.48, 0.12), vec3(1.0, 0.70, 0.18), t);
                    ...
                    // 阶段 6：晚霞落日熔金 (0.75 ~ 0.90) - 晚霞散射加剧，增加晚霞值与暖金赤橙霞光 (3600K ~ 2800K)
                    float t = (p - 0.75) / 0.15;
                    colCore  = mix(vec3(1.0, 0.97, 0.88), vec3(1.0, 0.94, 0.82), t);
                    colMid   = mix(vec3(1.0, 0.78, 0.32), vec3(1.0, 0.45, 0.12), t);
                    colOuter = mix(vec3(1.0, 0.58, 0.18), vec3(0.96, 0.26, 0.16), t);
                    rayScale = mix(0.40, 0.10, t);
                    k1 = mix(22.0, 16.0, t);
                    k2 = mix(90.0, 50.0, t);
                } else {
                    // 阶段 7：暮色沉落天际 (0.90 ~ 1.00) - 显著增加晚霞值，呈现浓郁壮丽晚霞紫红赤橙与暮色霞光 (2200K)
                    float t = (p - 0.90) / 0.10;
                    colCore  = mix(vec3(1.0, 0.94, 0.82), vec3(1.0, 0.90, 0.76), t);
                    colMid   = mix(vec3(1.0, 0.45, 0.12), vec3(0.96, 0.24, 0.08), t);
                    colOuter = mix(vec3(0.96, 0.26, 0.16), vec3(0.85, 0.14, 0.28), t);
